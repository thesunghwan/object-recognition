...
for i = 1:size(image_paths, 1)
    ...
    %feature = transpose(SIFT_features);
    features = [features SIFT_features];
    
%{ 
for m = 1:window_size:size(image, 1)-window_size
       for n = 1:window_size:size(image, 2)-window_size
           cropped_image = image(m:m+window_size-1, n:n+window_size-1);
           [locations, SIFT_features] = vl_dsift(cropped_image);
